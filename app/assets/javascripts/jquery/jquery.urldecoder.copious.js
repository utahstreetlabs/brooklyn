 ...
 *
 *
 *      $.url.extend('http://username:password@hostname/path?querystring'
 *                   {
 *                    'arg1':'value1',
 *                    'arg2':'value2'
 *                   })
 *      // returns new params merged into query string, updating any query string that might exist
 *      // i.e.
 *      // 'http://username:password@hostname/path?querystring&arg1=value1&arg2=value2'
 * ---------------------------------------------------------------------------
 * OTHER PARTIES' CODE:
 *
...
jQuery.url = function(){
  ...
  /**
   * private function to update url with params
   *
   * @param {String} url_str //required
   * @param {Hash} params
   * @return {String}
   */
  function extend(url_str, params) {
    url_str = url_str || window.location;
    params = params || {};

    var parsed = parse(url_str);
    if (parsed) {
      delete parsed['source'];
      delete parsed['query'];
      parsed['params'] = $.extend(parsed['params'], params);
      url_str = build(parsed);
    }
    return url_str;
  }
